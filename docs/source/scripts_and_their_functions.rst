...
DeDuplicator Node
-----------------
This node addresses the issues raised upon ReSequencing, where ``style`` and
``region`` elements and attributes are duplicated. To carry out the deduplication,
the DeDuplicator needs to take in a ``manifest_file`` e.g. ``manifest_ReSequenced2.txt``
