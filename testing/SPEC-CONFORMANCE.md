...
|---|---|---|---|---|
| | **Tech3370**| | |
 R1|2.2|When presenting a sequence of documents, at each moment in time exactly zero or one document shall be active.| Covered by tests for  R16, R17|
 R2|2.2 |If no document is active, or if a document with no content is active, no content shall be displayed. | ???|
 R3|2.2 |Sequences shall be considered distinct if they have had processing applied, even if the result of that processing is no change other than a known difference in state, for example if the processing has checked the spelling of the text content.|TBC dependent on passive node definition (e.g. change of white space character)|
 R4|2.2 |Every distinct sequence shall have a unique sequence identifier.|TBC dependent on above. |
 R5|2.2 |A document shall be associated with exactly one sequence.|Given seq1, seq2 and a document is added to seq1 and a document is added to seq2 then...|
...
R10|2.2 |~~Every document in a sequence shall be valid and self-contained.~~ |See https://git.ebu.io/ebutt/ebutt-part-3/issues/142|
...
R12|2.2 |A passive node shall NOT modify input sequences and shall only emit sequences that are identical (including the sequence numbers) to the input sequence(s)| `testing/bdd/features/nodes/passive_nodes_shall_not_modify_document.feature`|
R13|2.2 |At any moment in the presentation of a sequence by a node exactly zero or one document shall be temporally active.| Same as R1|
R14|2.3.1|At any single moment in time during the presentation of a sequence either zero documents or one document shall be active.| Same as R1|
R15|2.3.1 |The period during which a document is active begins at the document resolved begin time and ends at the document resolved end time.| Do we need a test for this? It will essentially test the calculation of resolved begin and end times.|
...
R21|2.3.4|~~A Delay node Retiming delay node shall NOT emit an output sequence with reordered subtitles.~~ Replaced by R116.| |
R22|2.3.4|~~A Delay node shall NOT update the value of `ebuttm:authoringDelay`.~~ Replaced by R117| TODO|
...
R24|2.4.1|All documents within a sequence that contain the element `ebuttp:authorsGroupIdentifier` shall have the same `ebuttp:authorsGroupIdentifier`.| `bdd/features/handover/group_identifier.feature`|
...
R33|3.2.2.1|The `ttp:timeBase` element is as defined in [EBUTT1] with the addition that all time expressions of `dur` attributes shall denote a relative coordinate on the same timeline as the `begin` and `end` attributes.| DONE?|
...
R40|3.2.2.2|`ebuttm:documentFacet` : Each distinctly identified `facet` that is summarised shall have a separate `documentFacet` element.| TODO|
R41|3.2.2.2|Documents shall NOT contain more than one `documentFacet` element referring to the same term, where the term is identified by the combination of the element contents and the value of the link attribute.| TODO|
...
R44|3.2.2.3| A document that contains a `tt:body` element with no content shall be treated as being active as defined by the semantics described in § 2.3.1, and shall cause no content to be presented while it is active.| TODO|
...
R75|3|If the `cell` unit is used, `ttp:cellResolution` SHALL be specified. |TODO: but  R74 sets a default value |
R76|3|If the `pixel` unit is used, `tts:extent` SHALL be specified on `tt:tt`|TODO|
...
R79|3.1.1.1.8|If specified, `ebuttm:documentTargetActiveFormatDescriptor` SHALL have one of the AFD codes specified in SMPTE ST 2016-1:2009|TODO|
R80|3.1.1.1.9|If specified, `ebuttm:documentIntendedTargetBarData` SHALL be specified in accordance with SMPTE ST2016-1:2009 Table 3|TODO|
R81|3.1.1.1.9|If specified, `ebuttm:documentIntendedTargetBarData` SHALL have the attribute `position` specified.|TODO|
R82|3.1.1.1.9|If the `position` attribute has the value `topBottom` then the `ebuttm:documentIntendedTargetBarData` element SHALL also contain the `lineNumberEndOfTopBar` and `lineNumberStartOfBottomBar` attributes.|TODO|
R83|3.1.1.1.9|If the `position` attribute has the value `leftRight` then the `ebuttm:documentIntendedTargetBarData` element SHALL also contain the `pixelNumberEndOfLeftBar` and `pixelNumberStartOfRightBar` attributes.|TODO|
R84|3.1.3.2|`tt:styling` SHALL contain one or more `tt:style` elements|TODO (already in XSD?)|
R85|3.1.4.2|A style attribute that is defined for `tt:region` SHALL NOT appear in `tt:style` and vice versa. The only exception from this rule is the `tts:padding`, which may appear in both `tt:style` and `tt:region`.|TODO|
R86|3.1.3.2|`tt:div`, `tt:p` and `tt:span` elements SHALL only use references to style definitions.|XSD?|
R87|3.1.3.2|The default value of `tts:fontSize` SHALL be `1c` (but `1c 2c` for v1.0)|TODO|
R88|3.1.3.2|The default value of `tts:textAlign` SHALL be `start`|TODO|
R89|3.1.4.2|`tt:layout` SHALL contain one or more `tt:region` elements|XSD?|
R90|3.1.4.2|The attributes `tts:origin` and `tts:extent` SHALL be specified on `tt:region`|XSD?|
R91|3.1.4.2|If a region exceeds the boundary of the active video, the display of the region SHALL be clipped.|PROCESSOR?|
R92|3.1.4.2|The initial value of `tts:displayAlign` SHALL be `before`|TODO|
...
R109|2.3.4.1|A Delay node SHALL accept a non-negative offset value |`bdd/features/timing/bufferDelayNode.feature`|
R110|2.3.4.1|~~A Buffer Delay node SHALL NOT alter the sequence identifier~~TBC (may be superseded by a more general requirement)| `bdd/features/timing/delay.feature`|
...
R113|2.3.4.2|A Retiming Delay node SHALL output a sequence with an identifier different to that of the input.  | TODO|
R114|2.3.4.2|A Retiming Delay node SHALL modify each document to result in the document’s computed times being offset by the offset period.  |`bdd/features/timing/delay.feature`|
R115|2.3.4.2|A Retiming Delay node SHALL NOT delay emission of the stream additionally to the time required to modify the times within the document.|TODO|
R116|2.3.4.2|A Retiming Delay node shall not emit an output sequence with reordered subtitles. |TODO|
R117|2.3.4.2|A Retiming Delay node shall not update the value of `ebuttm:authoringDelay`. |TODO|