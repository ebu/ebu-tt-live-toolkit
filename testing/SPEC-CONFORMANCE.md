...
||| **Tech 3370 v1.0**||
|R120|2.4.2|The Handover Manager shall maintain a token `T` equal to the value of the `ebuttp:authorsGroupControlToken` of the most recently emitted document, or a null value if no document has been emitted.||
|R121|2.4.2|The Handover Manager shall maintain a record `Ss` of the sequence identifier of the source sequence used to generate the most recently emitted document, known as the selected sequence, initially a null value until a sequence is selected.||
|R122|2.4.2|The Handover Manager shall maintain a record `AG` of the configured authors group identifier.||
|R122|2.4.2|The Handover Manager shall maintain an output sequence identifier `So`.||
|R123|2.4.2|Valid input for the Handover Manager is a valid Part 3 document that contains a `ebuttp:authorsGroupIdentifier` equal to the configured authors group identifier and an `ebuttp:authorsGroupControlToken`|No test (definition)|
|R123|2.4.2|On receiving a valid input document, if `T` is null or `T` is not null and the input document’s `ebuttp:authorsGroupControlToken` is greater than `T`, the Handover Manager shall execute the following sub steps in this order: 1) set `T` to the input document’s `ebuttp:authorsGroupControlToken`; 2) set `Ss` to the input document’s sequence identifier.|`bdd/features/handover/handover_algorithm.feature`|
|R123|2.4.2|On receiving a valid input document, if and only if the input document’s sequence identifier equals `Ss` then execute the following steps in this order: 1) generate an output document based on the input document, setting the output document’s sequence identifier to `So` and allocating a valid sequence number greater than the most recently emitted sequence number, and adding an `ebuttm:authorsGroupSelectedSequenceIdentifier` attribute set to `Ss`; 2) emit the output document. |`bdd/features/handover/handover_algorithm.feature`|
|R124|2.4.2|A Handover manager shall include within each output document the metadata attribute `authorsGroupSelectedSequenceIdentifier` set to the value of the input document's sequence identifier.|`bdd/features/handover/handover_algorithm.feature`|
|R125|3.2.2.1|The data type of `ebuttp:authorsGroupControlToken` shall be a positive integer.|`xsd/ebutt_parameters.xsd`|
|R126|3.2.2.1|The data type of `ebuttp:sequenceNumber` shall be a positive integer.|`xsd/ebutt_parameters.xsd`|
|R127|3.2.2.1|The data type of `ebuttp:sequenceIdentifier` shall be a string of length >= 1.|`bdd/features/validation/sequence\_id\_num.feature` `(In)valid Sequence head attributes`|
|R128|3.2.2.1|The data type of `ebuttp:authorsGroupIdentifier` shall be a string of length >= 1.|`bdd/features/handover/handover.feature`|
