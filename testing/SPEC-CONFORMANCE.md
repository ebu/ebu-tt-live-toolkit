...
| R8|2.2 |Every distinct document with the same sequence identifier shall have a different sequence number.|`bdd/features/validation/sequence\_id\_num.feature` `(In)valid sequence number for documents in the same sequence`|
...
|R37|3.2.2.1|Every document with the same `ebuttp:sequenceIdentifier` shall be uniquely numbered using the `ebuttp:sequenceNumber` attribute.|`bdd/features/validation/sequence\_id\_num.feature` `(In)valid sequence number for documents in the same sequence`|
