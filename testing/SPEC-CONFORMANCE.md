...
||| **Tech 3370 v1.0**||
|R120|2.4.2|Valid input for the Handover Manager is a valid document that contains all of: 1) a `ebuttp:authorsGroupIdentifier` equal to the configured authors group identifier; and 2) an `ebuttp:authorsGroupControlToken`|No test (definition)|
|R121|2.4.2|A Handover Manager shall not emit any documents derived from invalid input sequence documents.|`bdd/features/handover/handover.feature`|`bdd/features/handover/handover_algorithm.feature`|
|R122|2.4.2|If the sequence identifier of a valid Handover Manager input document is the selected sequence identifier, or if no sequence has yet been selected, the Handover Manager shall: 1) generate an output document based on the incoming document; 2) emit the output document; 3) record the value of `ebuttp:authorsGroupControlToken` from the input document; 4) record the value of `ebuttp:sequenceIdentifier` from the input document.|`bdd/features/handover/handover_algorithm.feature`|
|R123|2.4.2|If the sequence identifier of a valid Handover Manager input document is not the selected sequence identifier, the Handover Manager shall not emit an output document.|`bdd/features/handover/handover_algorithm.feature`|
|R124|2.4.2|A Handover Manager shall set its output document’s sequence identifier to the Handover Manager’s output sequence identifier.|`bdd/features/handover/handover_algorithm.feature`|
|R125|2.4.2|A Handover Manager shall set its output document's sequence number to a valid value greater than the most recently emitted sequence number.|`bdd/features/handover/handover_algorithm.feature`|
...
|R127|2.4.2|A Handover manager shall record the value of `ebuttp:authorsGroupControlToken` from the input document for next time the handover algorithm runs.|`bdd/features/handover/handover_algorithm.feature`|
|R128|2.4.2|A Handover manager shall record the value of `ebuttp:sequenceIdentifier` from the input document for next time the handover algorithm runs.|`bdd/features/handover/handover_algorithm.feature`|
|R129|3.2.2.1|The data type of `ebuttp:authorsGroupControlToken` shall be a positive integer.|`xsd/ebutt_parameters.xsd`|
|R130|3.2.2.1|The data type of `ebuttp:sequenceNumber` shall be a positive integer.|`xsd/ebutt_parameters.xsd`|
|R131|3.2.2.1|The data type of `ebuttp:sequenceIdentifier` shall be a string of length >= 1.|`bdd/features/validation/sequence\_id\_num.feature` `(In)valid Sequence head attributes`|
|R132|3.2.2.1|The data type of `ebuttp:authorsGroupIdentifier` shall be a string of length >= 1.|`bdd/features/handover/handover.feature`|
|R133|2.4.2|The Handover Manager shall maintain all of the following variables: a token equal to the value of the `ebuttp:authorsGroupControlToken` of the most recently emitted document, or a null value if no document has been emitted; a record of the sequence identifier of the source sequence used to generate the most recently emitted document, known as the selected sequence, initially a null value until a sequence is selected; a record of the configured authors group identifier; an output sequence identifier.|Tested implicitly as part of the algorithm in `bdd/features/handover/handover_algorithm.feature`|
